...
public class BuildingsController {
    ...
    // todo 查询所有城市为下拉菜单  供用户选择
    @GetMapping("/cities")
    @Operation(summary = "查询所有城市下拉列表")
    public Result<List<String>> getCityList() {
        // select distinct city from dorm_building
        return null;
    }


    // todo  根据所在城市查询该城市的校区,没有则查询所有校
    @GetMapping("/campuses")
    ...
    // todo 根据校区,查询该校区ID查询该校区的所有片区,然后下拉菜单用户选择 ,还未选择校区时查询所有片区
    ...
    // todo //根据片区查询楼栋编码和名称为下拉菜单
    @GetMapping("/buildings/codes")
    @Operation(summary = "根据片区查询楼栋编码和名称为下拉列表")
    public Result<List<BuildingSimpleVO>> getBuildingCodeList(@RequestParam(required = false) Long areaId) {
        return null;
    }
